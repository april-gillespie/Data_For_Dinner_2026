...
- Preserved all eight supplied files byte for byte. The [artifact manifest](artifact_manifest.json) records sizes, SHA-256 hashes, and Git blob hashes.
- Checked the original SRT: 106 sequential blocks, 62 containing text, 44 empty, positive durations, and no overlaps. The final timestamp is 00:06:38,500.
- Generated the readable transcript directly from the nonempty SRT blocks without correcting source wording.
...
It checks original-file sizes and hashes, caption structure, exact transcript reconstruction, the archived file hashes, and final Markdown local file targets. It excludes historical Markdown from current-link checks because that content is preserved as a snapshot. External URLs and Markdown anchor fragments are not validated by the script.
...
- The final video has not been supplied. Playback, audio, duration, and caption alignment await the final recording. See the [video home](../video/README.md).
